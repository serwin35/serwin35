...
const HIGHLIGHT_TERMS = [
    "Laravel", "PHP", "Vue.js", "React", "AWS", "Docker",
    "CI/CD", "DevOps", "CRM/ERP", "Livewire",
]

/** Split bio text into highlighted React segments — no innerHTML */
function HighlightedBio({ text }: { text: string }) {
    const pattern = new RegExp(`(${HIGHLIGHT_TERMS.map((t) => t.replace(/[.+]/g, "\\$&")).join("|")})`, "g")
...
export default function About() {
    ...
    const ghStats = [
        ...
        { value: ghCommits?.total_count != null ? `${ghCommits.total_count.toLocaleString()}+` : "—",
          label: t("about.ghCommits") },
        { value: ghPRs?.total_count != null ? `${ghPRs.total_count.toLocaleString()}+` : "—",
          label: t("about.ghPRs") },
    ]
        ...
        >
            ...
            <div className="bento-grid mb-8">
                ...
                {stats.map((stat, i) => (
                    ...
                    >
                        <span className="text-3xl font-black mb-1" style={{ color: stat.accent }}>
                            {stat.value}
                        </span>
                ...
                >
                    <div className="flex items-center gap-2.5 mb-5">
                        <svg className="w-5 h-5" style={{ color: "var(--color-accent)" }} fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-5">
                        {ghStats.map((item, i) => (
                            ...
                            >
                                <span className="block text-xl font-bold" style={{ color: "var(--color-accent-light)" }}>
                                    {item.value}
                                </span>
                ...
                >
                    <p className="text-xs font-semibold uppercase tracking-widest mb-4" style={{ color: "var(--color-violet)" }}>
                        {t("Personal Information")}
                    </p>
                    <div className="space-y-4">
                        {[
                            { label: "E-mail", value: "mateusz.serwinowski@gmail.com", href: "mailto:mateusz.serwinowski@gmail.com" },
                            { label: t("Phone"), value: "+48 576-721-998", href: "tel:+48576721998" },
                        ].map((item) => (
                            <div key={item.label}>
                                <p className="text-xs mb-1 uppercase tracking-wider" style={{ color: "var(--color-text-muted)" }}>
                                    {item.label}
                                </p>
                                <a
                                    href={item.href}
                ...
                >
                    ...
                    <div className="space-y-3">
                        <div className="flex items-center gap-2">
                            <svg className="w-4 h-4 shrink-0" style={{ color: "var(--color-accent)" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                                <path strokeLinecap="round" strokeLinejoin="round" d="M15 10.5a3 3 0 11-6 0 3 3 0 016 0z" />
                                <path strokeLinecap="round" strokeLinejoin="round" d="M19.5 10.5c0 7.142-7.5 11.25-7.5 11.25S4.5 17.642 4.5 10.5a7.5 7.5 0 1115 0z" />
                            </svg>
                            <span className="text-sm" style={{ color: "var(--color-text-secondary)" }}>Lodz, Poland</span>
                        </div>
                        <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-sm" style={{ color: "var(--color-text-muted)" }}>Volunteer Firefighter</span>
                        </div>
                    </div>
                </motion.div>
            </div>
